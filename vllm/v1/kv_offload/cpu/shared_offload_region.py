    ...
    def __init__(
        ...
        try:
            ...
            bind_memory_to_numa_node(
                self.mmap_obj,
                self.mapped_size_bytes,
                self.memory_config.numa_node,
            )
            if self.memory_config.prefault:
                self._prefault(rank, cpu_page_size)
        except Exception:
            self.cleanup()
            raise
        ...
        self._base = torch.frombuffer(
            memoryview(self.mmap_obj),
            dtype=torch.int8,
            count=self.total_size_bytes,
        )
        self._canonical_offset = 0

    def _prefault(self, rank: int | None, cpu_page_size: int) -> None:
        assert self.mmap_obj is not None
        populate_write_fn = _get_populate_write_fn(self.mmap_obj)
        if self.memory_config.effective_backend == CPUOffloadMemoryBackend.HUGETLBFS:
            start = time.perf_counter()
            populate_write_fn(self.mmap_obj, 0, self.mapped_size_bytes)
            logger.debug(
                "Prefaulted hugetlbfs CPU region in %.3f s",
        ...
        if rank is not None:
            ...
            logger.debug(
                "Prefaulted %d worker block ranges in %.3f s",
                self.num_blocks,
                time.perf_counter() - start,
            )
            return
        ...
        logger.debug(
            "Prefaulted entire shared CPU region in %.3f s",
            time.perf_counter() - start,
        )
