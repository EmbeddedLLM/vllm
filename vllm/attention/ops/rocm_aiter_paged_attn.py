...
def rocm_aiter_reshape_and_cache_with_pertoken_quant_impl(
    key: torch.Tensor,
    value: torch.Tensor,
...
def rocm_aiter_pa_fwd_asm_inplace_impl(
    output: torch.Tensor,
    query: torch.Tensor,
    key_cache: torch.Tensor,
    value_cache: torch.Tensor,
    ...
    rocm_aiter.pa_fwd_asm(query, key_cache, value_cache, block_tables,
                          seq_lens, max_num_blocks_per_seq, k_scale, v_scale,
                          output)


def rocm_aiter_pa_fwd_asm(output: Optional[torch.tensor], query: torch.Tensor,
                          key_cache, value_cache, block_tables, seq_lens,
                          max_seq_len, kv_cache_dtype, k_scale,
                          v_scale) -> torch.Tensor:
    if output is None:
        output = torch.empty_like(query)

    rocm_aiter_pa_fwd_asm_inplace_impl(output=output,
                                       query=query,
                                       key_cache=key_cache,
...
class AITERPagedAttention(PagedAttention):
    ...
    def write_to_paged_cache(
        ...
        if kv_cache_dtype not in ["int8", "fp8", "fp8_e4m3"]:
            ...
            rocm_aiter_reshape_and_cache_with_pertoken_quant_impl(
