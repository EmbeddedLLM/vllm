...
def _atom_can_skip_legacy_decode_metadata() -> bool:
    """Whether pure-decode legacy sparse metadata is dead for this deployment.

    The ATOM model-state decode path builds SWA/CSA/HCA indices from request
    state and block tables. The legacy C128A dense/ragged metadata is only
    needed by the fallback sparse attention path and by the native-HCA debug
