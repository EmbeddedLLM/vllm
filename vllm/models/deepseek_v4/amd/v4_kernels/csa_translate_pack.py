...
Correctness:
  - paged_decode reads `kv_indices_csa[indptr[t] : indptr[t+1]]` whose length
    is exactly `skip_prefix_len_per_token[t] + valid_k[t]`. Decode writes the
    CSA topk section at the slice HEAD `[indptr[t], indptr[t]+valid_k[t])`,
    where `valid_k[t]` is recovered as `indptr[t+1] - indptr[t] - skip[t]`;
    the SWA prefix (length `skip[t]`) occupies the tail, written by
    `write_v4_paged_decode_indices`.
  - paged_prefill uses the same packed length, but `write_v4_paged_prefill_indices`
    writes SWA prefix at the slice head. In that branch this kernel writes CSA
    topk after the prefix at `[indptr[t]+skip[t], indptr[t]+skip[t]+valid_k[t])`.
...
def _csa_translate_pack_kernel(
    ...
    # Decode keeps CSA topk at the slice head and SWA at the tail. Prefill keeps
    # prior-SWA prefix at the head, so CSA topk starts after `skip`.
    write_base = tl.where(INLINE_SKIP_FROM_POS, indptr_t, indptr_t + skip)
    ...
    *,
    ...
    Args:
      ...
      skip_prefix_len_per_token:   [T] int32 OR None — per-token SWA prefix
                                   length. Decode treats this as the tail
                                   segment length; prefill treats it as the
                                   head prefix length and topk write offset.
                                   Pass None
      ...
      kv_indices_csa:              [total_indices] int32 — destination buffer;
                                   decode writes the CSA topk section at the
                                   slice head `[indptr[t], indptr[t]+valid_k[t])`;
                                   prefill writes it after the SWA prefix at
...
def csa_translate_pack_reference(
    ...
    *,
    ...
    for t in range(T):
        ...
        write_base = base if inline_skip else base + skip_t
        ...
        for k in range(valid_k):
            kv_indices_csa[write_base + k] = int(paged[k].item())
