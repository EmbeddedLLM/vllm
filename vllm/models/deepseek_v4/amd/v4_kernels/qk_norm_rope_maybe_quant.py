...
Replaces this 3-kernel sequence on the V4 decode path::
    ...
    rotary_emb(
        positions,
        q[..., -rd:],  # aiter: rope_cached_positions_2c
        kv[..., -rd:],
    )
...
def qk_norm_rope_maybe_quant(
    ...
    """
    assert q.dim() == 2 and kv.dim() == 2, (
        f"q/kv must be 2-D; got q={tuple(q.shape)} kv={tuple(kv.shape)}"
    )
    T = q.shape[0]
    assert q.shape[1] == n_local_heads * head_dim, (
    ...
    assert rope_head_dim <= head_dim and rope_head_dim % 2 == 0, (
    ...
    assert head_dim % rope_head_dim == 0, (
