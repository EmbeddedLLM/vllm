...
def make_compress_plans(
    ...
    *,
    ...
    write_capacity_per_ratio: dict[int, int] | None = None,
    ...
    Args:
      ...
      write_capacity_per_ratio: optional dict[ratio] -> int. Slice length for
                    `write_plan_gpu`. When PROVIDED, only that fixed-capacity
                    prefix is sentinel-filled and copied to GPU. This is the
                    decode graph analogue of `decode_capacity_per_ratio` for
                    the state-update plan; prefill keeps the full buffer.
    ...
    if total == 0 or bs == 0:
        ...
        for ratio, _ in unique_ratios_overlap:
            ...
            write_cap = (
                write_capacity_per_ratio.get(ratio)
                if write_capacity_per_ratio is not None
                else wbuf.np.shape[0]
            )
            if cap > 0:
                cbuf.np[:cap].fill(-1)
            if write_cap > 0:
                wbuf.np[:write_cap].fill(-1)
            compress_plan_gpu = cbuf.copy_to_gpu(cap if cap > 0 else 0)
            out[ratio] = CompressPlan(
                compress_plan_gpu=compress_plan_gpu,
                write_plan_gpu=wbuf.copy_to_gpu(write_cap),
                num_compress=0,
                num_write=0,
    ...
    for ratio, is_overlap in unique_ratios_overlap:
        ...
        assert n_write <= write_cap <= wbuf.np.shape[0], (
            f"ratio={ratio} num_write={n_write} exceeds buffer "
            f"capacity {write_cap}; bump in builder __init__."
        )
        if n_compress > 0:
        ...
        wbuf.np[n_write:write_cap].fill(-1)  # sentinel
