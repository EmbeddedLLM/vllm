...
def _fused_compress_attn_kernel(
    ...
    cache_scale_token_stride,
    block_table_ptr,  # [bs, max_blocks_per_seq] int32
    block_table_seq_stride,  # row stride
    kv_slot_mapping_ptr,  # [num_tokens] optional direct scatter map
    k_per_block,
    head_dim,
    rope_head_dim,
    # ── constexpr ───────────────────────────────────────────────────────
    ...
    # the only output.
    if HAS_BLOCK_TABLE or HAS_SLOT_MAPPING:
        # Main CSA/HCA uses block_table[batch_id, ci // k_per_block].
        # The indexer-inner path already has vLLM's compressed slot_mapping,
        # so scatter directly by ragged_id for mixed decode/prefill batches.
        ci = position // RATIO
        slot_valid = True
        if HAS_SLOT_MAPPING:
            kv_slot = tl.load(kv_slot_mapping_ptr + ragged_id)
            slot_valid = kv_slot >= 0
            safe_slot = tl.where(slot_valid, kv_slot, 0)
            physical_block = (safe_slot // k_per_block).to(tl.int64)
            slot_in_block = safe_slot % k_per_block
        else:
        ...
        store_mask = d_mask & slot_valid

        if PACKED_FP8_DS_MLA:
            # ATOM DSV4 packed fp8_ds_mla layout:
            #   block data region:  k_per_block * 576 bytes
            #     per token:        448 FP8 NoPE bytes + 64 BF16 RoPE values
            #   block scale region: k_per_block * 8 UE8M0 scale bytes
            # The tensor is shaped [NB, k_per_block, 584] as a byte container,
            ...
            token_scale_base = (
                physical_block * kv_cache_block_stride
                + k_per_block * TOKEN_DATA_SIZE
                + slot_in_block * TOKEN_SCALE_DIM
            )

            is_fp8_dim = d < TOKEN_FP8_DIM
            fp8_abs = tl.where(is_fp8_dim & d_mask, tl.abs(rotated), 0.0)
            for g in tl.static_range(0, TOKEN_FP8_DIM // 64):
                group_mask = (d >= g * 64) & (d < (g + 1) * 64)
                amax = tl.max(tl.where(group_mask, fp8_abs, 0.0), axis=0)
                raw_scale = tl.maximum(amax, 1e-4) * (1.0 / FP8_MAX)
                exponent = tl.ceil(tl.log2(raw_scale))
                scale_g = tl.exp2(exponent)
                scaled = tl.clamp(rotated / scale_g, -FP8_MAX, FP8_MAX)
                fp8_val = scaled.to(tl.float8e4nv).to(tl.uint8, bitcast=True)
                tl.store(
                    kv_cache_ptr + token_data_base + d,
                    fp8_val,
                    mask=store_mask & group_mask,
                    cache_modifier=".cs",
                )
                encoded_scale = tl.maximum(tl.minimum(exponent + 127.0, 255.0), 0.0)
                tl.store(
                    kv_cache_ptr + token_scale_base + g,
            ...
            # Padding scale byte.
            tl.store(
                kv_cache_ptr + token_scale_base + 7,
                tl.zeros((), dtype=tl.uint8),
                mask=slot_valid,
                cache_modifier=".cs",
            )

            # Store the RoPE tail as BF16 at byte offset 448.
            bf16_offsets = tl.maximum(d - TOKEN_FP8_DIM, 0)
            bf16_ptr = (kv_cache_ptr + token_data_base + TOKEN_FP8_DIM).to(
                tl.pointer_type(tl.bfloat16)
            )
            tl.store(
                bf16_ptr + bf16_offsets,
                rotated.to(tl.bfloat16),
                mask=store_mask & (d >= TOKEN_FP8_DIM) & (d < head_dim),
                cache_modifier=".cs",
            )
        elif QUANT:
            # FP8 e4m3 quantization: per-row or per-1x64 amax → ue8m0
            # (or raw) scale → clamp+cast to fp8 → write preshuffled
            # (MFMA 16x16 tile) or linear layout into the FP8 region; write
            # fp32 scale(s) into the paired `cache_scale_ptr` view.
            ...
            group_idx = d // QUANT_GROUP_SIZE
            scale = tl.zeros([BLOCK_D], tl.float32)
            for g in tl.static_range(0, BLOCK_D // QUANT_GROUP_SIZE):
                group_mask = (group_idx == g) & d_mask
                amax = tl.max(tl.where(group_mask, rotated_for_amax, 0.0), axis=0)
                scale_g = tl.maximum(amax, 1e-4) * (1.0 / FP8_MAX)
                if USE_UE8M0:
                    scale_g = tl.exp2(tl.ceil(tl.log2(scale_g)))
                scale = tl.where(group_mask, scale_g, scale)
                scale_offset = (
                    physical_block * cache_scale_block_stride
                    + slot_in_block * cache_scale_token_stride
                    + g
                )
                tl.store(
                    cache_scale_ptr + scale_offset,
                    scale_g,
                    mask=slot_valid,
                    cache_modifier=".cs",
                )
            inv_scale = tl.where(d_mask, 1.0 / scale, 1.0)
            ...
            tl.store(
                kv_cache_ptr + cache_addr,
                rotated.to(tl.bfloat16),
                mask=store_mask,
            )


def _validate_packed_fp8_ds_mla_fused_compress_args(
    *,
    kv_cache: torch.Tensor | None,
    cache_scale: torch.Tensor | None,
    has_scatter_map: bool,
    head_dim: int,
    rope_head_dim: int,
    k_per_block: int,
    fp8_max: float | None,
) -> None:
    if not has_scatter_map:
        raise RuntimeError(
            "packed_fp8_ds_mla=True requires block_tables or kv_slot_mapping"
        )
    if kv_cache is None:
        raise RuntimeError("packed_fp8_ds_mla=True requires kv_cache")
    ...
    if (
        ...
        raise RuntimeError(
            "packed fp8_ds_mla expects uint8 [num_blocks, k_per_block, 584], "
            f"got dtype={kv_cache.dtype}, shape={tuple(kv_cache.shape)}"
        )
    if head_dim != 512:
        raise RuntimeError(f"packed fp8_ds_mla expects head_dim=512, got {head_dim}")
    if rope_head_dim != 64:
        raise RuntimeError(
            f"packed fp8_ds_mla expects rope_head_dim=64, got {rope_head_dim}"
        )
    if k_per_block <= 0:
        raise RuntimeError(
            f"packed fp8_ds_mla expects k_per_block > 0, got {k_per_block}"
        )
    if fp8_max is not None and fp8_max <= 0:
        raise RuntimeError(f"packed fp8_ds_mla expects fp8_max > 0, got {fp8_max}")
...
def fused_compress_attn(
    *,
    ...
    assert cos_cache.stride(-1) == 1, (
        f"cos_cache inner stride {cos_cache.stride(-1)} must be 1"
    )
    assert sin_cache.stride(-1) == 1, (
        f"sin_cache inner stride {sin_cache.stride(-1)} must be 1"
    )
    ...
    if packed_fp8_ds_mla:
        _validate_packed_fp8_ds_mla_fused_compress_args(
            kv_cache=kv_cache,
            cache_scale=cache_scale,
            has_scatter_map=has_bt or has_slot_mapping,
            head_dim=head_dim,
            rope_head_dim=rope_head_dim,
            k_per_block=k_per_block,
            fp8_max=fp8_max,
    ...
    if has_slot_mapping:
        assert kv_cache is not None and kv_cache.dim() == 3
        assert kv_slot_mapping.dim() == 1

    # Quant validation. quant=True is FP8 cache write (Indexer-inner path);
    # requires a valid scatter map AND a paired fp32 scale view of the same
    # allocation (see Compressor.forward for how to slice it).
    if not packed_fp8_ds_mla and quant:
        ...
        scale_groups = head_dim // quant_group_size
        if scale_groups == 1:
            assert cache_scale.dim() == 2 and cache_scale.shape[0] == kv_cache.shape[0]
            assert cache_scale.shape[1] >= k_per_block
        else:
            assert cache_scale.dim() in (2, 3)
            if cache_scale.dim() == 2:
                assert cache_scale.shape[0] >= kv_cache.shape[0] * k_per_block
                assert cache_scale.shape[1] >= scale_groups
            else:
                assert cache_scale.shape[0] == kv_cache.shape[0]
                assert cache_scale.shape[1] >= k_per_block
    ...
    if packed_fp8_ds_mla:
        cache_scale_arg = state_slot_mapping  # placeholder ptr (unused)
        cache_scale_block_stride_arg = 0
        cache_scale_token_stride_arg = 0
        fp8_max_arg = float(fp8_max or 448.0)
    elif quant:
        cache_scale_arg = cache_scale
        scale_groups = head_dim // int(quant_group_size or head_dim)
        if scale_groups == 1:
            cache_scale_block_stride_arg = cache_scale.stride(0)
            cache_scale_token_stride_arg = cache_scale.stride(1)
        elif cache_scale.dim() == 2:
            cache_scale_block_stride_arg = k_per_block * cache_scale.stride(0)
            cache_scale_token_stride_arg = cache_scale.stride(0)
        else:
            cache_scale_block_stride_arg = cache_scale.stride(0)
            cache_scale_token_stride_arg = cache_scale.stride(1)
    ...
    quant_group_size_arg = int(quant_group_size or head_dim)
    kv_slot_mapping_arg = kv_slot_mapping if has_slot_mapping else state_slot_mapping
    ...
    _fused_compress_attn_kernel[grid](
        ...
        sin_cache,
        cos_cache.stride(0),
        kv_cache if (has_bt or has_slot_mapping) else cos_cache,
        kv_cache.stride(0) if (has_bt or has_slot_mapping) else 0,
        kv_cache.stride(1) if (has_bt or has_slot_mapping) else 0,
        cache_scale_arg,
        cache_scale_block_stride_arg,
        cache_scale_token_stride_arg,
        ...
        HAS_SLOT_MAPPING=int(has_slot_mapping),
        QUANT=int(quant),
        PACKED_FP8_DS_MLA=int(packed_fp8_ds_mla),
        FP8_MAX=fp8_max_arg,
        USE_UE8M0=int(use_ue8m0),
        QUANT_GROUP_SIZE=quant_group_size_arg,
