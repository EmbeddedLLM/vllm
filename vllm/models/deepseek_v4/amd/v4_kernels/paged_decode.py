...
reference (``sparse_attn_ragged_torch``) within fp32-accumulation tolerance.
...
@triton.jit
def _aiter_pa_decode_sparse_reduce_zero_safe(
    m_partial_ptr,
    l_partial_ptr,
    acc_partial_ptr,
    attn_sink_ptr,
    ...
    """aiter pa_decode_sparse reduce with a guard for graph-padded tokens."""
    t = tl.program_id(0)
    ...
    if kv_len == 0:
        tl.store(
            out_ptr + out_offsets,
            tl.zeros((BLOCK_H, BLOCK_D), dtype=out_ptr.dtype.element_ty),
    ...
    alpha_split = tl.exp(m_p - m_max[None, :])
...
def sparse_attn_v4_paged_decode_kv_splits(
    T: int,
    H: int,
    *,
    block_h: int | None = None,
) -> tuple[int, str]:
    """Return the split-K decision used by ``sparse_attn_v4_paged_decode``.

    This is intentionally host-only and used for profiling/logging. Keep it in
    sync with the launcher's block_h and override logic.
    """
    if block_h is None:
...
def _alloc_decode_split_partials(
    ...
    def _alloc_torch_empty() -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
def _paged_decode_fused_kernel(
    ...
    for j in tl.range(0, num_tiles, num_stages=3):
        ...
        if TRUST_INDICES:
            ...
            slot_valid = valid & (slot >= 0) & (slot < total_pages)
            safe_slot = tl.minimum(tl.maximum(slot, 0), total_pages - 1)
        ...
        scores = tl.where(slot_valid[None, :], scores, neg_large)
...
def _paged_decode_split_kv_fused_kernel(
    ...
    kv_indptr_ptr,  # [N+1] int32
    csa_topk_ptr,  # [N, index_topk] int32 when FUSE_CSA_TOPK
    csa_block_table_ptr,  # [bs, mnbps] int32 when FUSE_CSA_TOPK
    csa_positions_ptr,  # [N] int when FUSE_CSA_TOPK or ORDERED_SPLIT
    csa_batch_id_ptr,  # [N] int32 when FUSE_CSA_TOPK
    attn_sink_ptr,  # [H]
    out_ptr,  # [N, H, D]
    q_stride_t,
    q_stride_h,
    ...
    csa_block_table_rows: tl.constexpr,
    out_stride_t,
    out_stride_h,
    out_stride_d,
    qk_scale,
    ...
    FUSE_CSA_TOPK: tl.constexpr,
    ORDERED_SPLIT: tl.constexpr,
    CSA_INDEX_TOPK: tl.constexpr,
    CSA_BLOCK_CAPACITY: tl.constexpr,
    CSA_WINDOW_SIZE: tl.constexpr,
    TRUST_INDICES: tl.constexpr,
):
    """Single-pass split-layout decode.

    Unified slot ids below ``swa_pages`` load from ``swa_kv``. The remaining
    slots load from ``compressed_kv[slot - swa_pages]``, optionally dequantized
    with 1xGROUP_SIZE block scales.
    """
    t = tl.program_id(0)
    pid_h = tl.program_id(1)
    ...
    kv_len = kv_end - kv_start
    if FUSE_CSA_TOPK:
        csa_bid = tl.load(csa_batch_id_ptr + t)
        csa_token_valid = csa_bid >= 0
        csa_pos = tl.load(csa_positions_ptr + t, mask=csa_token_valid, other=0)
        csa_skip = tl.minimum(csa_pos + 1, CSA_WINDOW_SIZE)
        csa_skip = tl.where(csa_token_valid, csa_skip, 0)
        csa_len = tl.maximum(kv_len - csa_skip, 0)
    elif ORDERED_SPLIT:
        # Decode index builder writes compressed entries at the head of each
        # ragged slice and the SWA window at the tail.  This split point lets
        # homogeneous tiles avoid masked-off loads from the other backing store.
        csa_pos = tl.load(csa_positions_ptr + t)
        csa_skip = tl.minimum(csa_pos + 1, CSA_WINDOW_SIZE)
        csa_len = tl.maximum(kv_len - csa_skip, 0)
        csa_bid = 0
        csa_token_valid = True
    else:
        csa_bid = 0
        csa_token_valid = True
        csa_len = 0
    ...
    for j in tl.range(0, num_tiles, num_stages=3):
        ...
        k_pos = k_start + k_offs
        valid = k_pos < kv_len
        slot_from_indices = tl.load(
            kv_indices_ptr + kv_start + k_pos,
            mask=valid,
            other=0,
        )
        if FUSE_CSA_TOPK:
            is_csa_topk = k_pos < csa_len
            csa_lane_valid = valid & is_csa_topk & csa_token_valid
            topk = tl.load(
                csa_topk_ptr + t * csa_topk_stride + k_pos,
                mask=csa_lane_valid & (k_pos < CSA_INDEX_TOPK),
                other=-1,
            )
            csa_block_idx = topk // CSA_BLOCK_CAPACITY
            csa_slot = topk - csa_block_idx * CSA_BLOCK_CAPACITY
            csa_block_valid = (
                csa_lane_valid
                & (topk >= 0)
                & (csa_bid >= 0)
                & (csa_bid < csa_block_table_rows)
                & (csa_block_idx >= 0)
            ...
            csa_phys = tl.load(
                csa_block_table_ptr
                + safe_csa_bid * csa_block_table_stride
                + safe_csa_block_idx,
                mask=csa_block_valid,
                other=-1,
            )
            csa_lane_valid = csa_block_valid & (csa_phys >= 0)
            slot_from_topk = swa_pages + csa_phys * CSA_BLOCK_CAPACITY + csa_slot
            slot = tl.where(is_csa_topk, slot_from_topk, slot_from_indices)
            source_valid = tl.where(is_csa_topk, csa_lane_valid, valid)
        else:
            slot = slot_from_indices
            source_valid = valid
        if TRUST_INDICES:
            slot_valid = source_valid
            is_swa = slot < swa_pages
            safe_swa_slot = tl.where(is_swa, slot, 0)
            tail_slot = slot - swa_pages
            safe_tail_slot = tl.where(is_swa, 0, tail_slot)
        else:
            slot_valid = source_valid & (slot >= 0) & (slot < total_pages)
            is_swa = slot < swa_pages
        ...
        tile_is_ordered = FUSE_CSA_TOPK or ORDERED_SPLIT
        tile_all_tail = tile_is_ordered and (k_start + BLOCK_K <= csa_len)
        tile_all_swa = tile_is_ordered and (k_start >= csa_len)
        ...
        if PACKED_TAIL:
            ...
            if tile_all_swa:
                ...
                if QUANT_TAIL:
                    ...
                    tail_kv = tail_raw.to(q.dtype) * scales_full
                else:
                    tail_kv = tail_raw
        kv = tl.where(is_swa[:, None], swa_kv, tail_kv)
...
def _paged_decode_split_kv_split_kernel(
    ...
    kv_indices_ptr,  # [total_indices] int32, unified slot ids
    kv_indptr_ptr,  # [N+1] int32
    csa_positions_ptr,  # [N] int when ORDERED_SPLIT
    ...
    """Split-K partial writer for split SWA/compressed KV layout.

    This mirrors ``_paged_decode_split_kernel`` but loads each unified slot
    from either the SWA prefix or compressed tail. It intentionally consumes
    pre-packed ``kv_indices`` only; fused CSA-topk translation stays on the
    single-pass kernel path.
    """
    t = tl.program_id(0)
    pid_h = tl.program_id(1)
    ...
    for j in tl.range(tile_start, tile_end, num_stages=3):
        ...
        tile_all_tail = ORDERED_SPLIT and (k_start + BLOCK_K <= csa_len)
        tile_all_swa = ORDERED_SPLIT and (k_start >= csa_len)
...
def _sparse_attn_v4_paged_decode_triton(
    ...
    if out is None:
        ...
        if out.shape != q.shape:
            raise RuntimeError(
                f"sparse_attn_v4_paged_decode out shape {tuple(out.shape)} "
                f"does not match q shape {tuple(q.shape)}"
            )
        if out.dtype != q.dtype or out.device != q.device:
            raise RuntimeError(
                "sparse_attn_v4_paged_decode out must match q dtype/device: "
                f"out=({out.dtype}, {out.device}), q=({q.dtype}, {q.device})"
            )
    ...
    m_partial, l_partial, acc_partial = _alloc_decode_split_partials(
        q, T, kv_splits, h_padded, D
    )
...
def sparse_attn_v4_paged_decode_reference(
    ...
    ``sparse_attn_ragged_torch``. Slow but correct for unit tests / dump-bisect.
...
def _dequantize_packed_fp8_ds_mla_reference(
    packed_kv: torch.Tensor,
    out_dtype: torch.dtype,
) -> torch.Tensor:
    """Dequantize ATOM/V4 packed 584-byte KV blocks to dense [pages, 512].

    The storage tensor is shaped ``[num_blocks, block_size, 584]`` only as a
    byte container. Semantically each block stores all token data first
    (`block_size * 576` bytes), followed by all per-token UE8M0 scales
    (`block_size * 8` bytes).
    """
    if packed_kv.dtype != torch.uint8:
        raise RuntimeError(f"packed fp8_ds_mla expects uint8, got {packed_kv.dtype}")
    if packed_kv.dim() != 3 or packed_kv.shape[-1] != 584:
        raise RuntimeError(
            "packed fp8_ds_mla expects [num_blocks, block_size, 584], got "
            f"{tuple(packed_kv.shape)}"
        )
    num_blocks, block_size, _ = packed_kv.shape
    block_bytes = packed_kv.reshape(num_blocks, -1)
    out = torch.empty(
        (num_blocks * block_size, 512),
        dtype=out_dtype,
        device=packed_kv.device,
    )
    fp8_dtype = getattr(torch, "float8_e4m3fn", _FP8_DTYPE)
    for block in range(num_blocks):
        for slot in range(block_size):
            ...
            scales = torch.exp2(encoded - 127.0).repeat_interleave(64)
            out[page, :_PACKED_FP8_DIM] = (fp8_vals * scales).to(out_dtype)
            bf16_start = data_start + _PACKED_FP8_DIM
            bf16_bytes = block_bytes[
                block,
                bf16_start : bf16_start + _PACKED_BF16_DIM * torch.bfloat16.itemsize,
            ]
            out[page, _PACKED_FP8_DIM:] = bf16_bytes.view(torch.bfloat16).to(out_dtype)
    return out


def sparse_attn_v4_paged_decode_split_kv_reference(
    q: torch.Tensor,
    swa_kv: torch.Tensor,
    ...
    *,
    ...
    """Reference split-layout decode for the target ATOM KV contract.

    This is intentionally not the production kernel. It makes the desired
    mixed layout explicit and testable:

    - unified slot ids ``[0, swa_pages)`` read BF16/FP16 SWA pages;
    - unified slot ids ``[swa_pages, ...)`` read compressed tail pages;
    - the compressed tail may be BF16/FP16, or FP8 with 1xGROUP_SIZE scales.

    The implementation materializes a temporary homogeneous KV pool and then
    calls ``sparse_attn_v4_paged_decode_reference``. The production follow-up
    is a Triton split-load kernel that selects SWA versus compressed-tail
    ...
    if swa_kv.shape[-1] != D:
        raise RuntimeError(
            f"swa_kv last dim {swa_kv.shape[-1]} does not match q dim {D}"
        )
    ...
    if packed_tail:
        if D != 512 or compressed_kv.dtype != torch.uint8:
            raise RuntimeError(
                "packed fp8_ds_mla reference expects q dim 512 and uint8 "
                f"compressed_kv, got D={D}, dtype={compressed_kv.dtype}"
            )
        if compressed_kv.dim() != 3 or compressed_kv.shape[-1] != 584:
            raise RuntimeError(
                "packed fp8_ds_mla compressed_kv must be "
                f"[num_blocks, block_size, 584], got {tuple(compressed_kv.shape)}"
            )
    ...
    if swa_flat.shape[0] != swa_pages:
        raise RuntimeError(
            f"swa_kv flattens to {swa_flat.shape[0]} pages, "
            f"expected swa_pages={swa_pages}"
        )
    if packed_tail:
        compressed_dequant = _dequantize_packed_fp8_ds_mla_reference(
            compressed_kv, q.dtype
        )
        compressed_flat_pages = compressed_dequant.shape[0]
    else:
        compressed_flat = compressed_kv.reshape(-1, D)
        compressed_flat_pages = compressed_flat.shape[0]
    ...
    if packed_tail:
    ...
    elif compressed_kv_scales is not None:
        if compressed_flat.dtype != _FP8_DTYPE:
            raise RuntimeError(
                "compressed_kv_scales supplied but compressed_kv dtype is "
        ...
        num_groups = D // _FP8_GROUP_SIZE
        scales = compressed_kv_scales.reshape(-1, num_groups)
        if scales.shape[0] != compressed_flat.shape[0]:
            raise RuntimeError(
                f"compressed_kv_scales has {scales.shape[0]} pages, "
                f"expected {compressed_flat.shape[0]}"
            )
        compressed_fp32 = compressed_flat.to(torch.float32)
        scales_expanded = (
            scales.view(compressed_flat.shape[0], num_groups, 1)
            .expand(compressed_flat.shape[0], num_groups, _FP8_GROUP_SIZE)
            .reshape(compressed_flat.shape[0], D)
        )
        compressed_dequant = (compressed_fp32 * scales_expanded).to(q.dtype)
    elif not packed_tail:
        if compressed_flat.dtype != q.dtype:
            raise RuntimeError(
                "compressed_kv dtype must match q when scales are absent: "
                f"compressed={compressed_flat.dtype}, q={q.dtype}"
            )
        compressed_dequant = compressed_flat

    unified_kv = torch.empty(
        (swa_pages + compressed_flat_pages, D),
        dtype=q.dtype,
        device=q.device,
    )
    unified_kv[:swa_pages].copy_(swa_flat)
    unified_kv[swa_pages:].copy_(compressed_dequant)
...
def sparse_attn_v4_paged_decode_split_kv(
    q: torch.Tensor,
    swa_kv: torch.Tensor,
    compressed_kv: torch.Tensor,
    kv_indices: torch.Tensor,
    ...
    *,
    ...
    csa_positions: torch.Tensor | None = None,
    csa_batch_id_per_token: torch.Tensor | None = None,
    csa_block_capacity: int = 0,
    csa_window_size: int = 0,
) -> torch.Tensor:
    """Deployment-shaped fused decode for split SWA/compressed KV layout.

    This is the production-kernel counterpart to
    ``sparse_attn_v4_paged_decode_split_kv_reference`` for the current
    deployment configuration where ``kv_splits == 1``. It avoids materializing
    ...
    if q.dtype not in (torch.bfloat16, torch.float16):
        raise RuntimeError(
            f"sparse_attn_v4_paged_decode_split_kv expects fp16/bf16 q, got {q.dtype}"
        )
    ...
    if quant_tail:
        ...
        num_groups = D // _FP8_GROUP_SIZE
        tail_scales = compressed_kv_scales.reshape(-1, num_groups)
    ...
    if out is None:
        ...
        raise RuntimeError("split KV decode out must match q shape/dtype/device")
    if T == 0:
        return out
    fuse_csa_topk = csa_topk_local is not None
    ordered_split = csa_positions is not None and csa_window_size > 0
    if fuse_csa_topk:
        if (
            csa_block_tables is None
            or csa_positions is None
            or csa_batch_id_per_token is None
            or csa_block_capacity <= 0
            or csa_window_size <= 0
        ):
            raise RuntimeError(
                "CSA top-k fused split-KV decode requires block tables, "
                "positions, batch ids, positive csa_block_capacity, and "
                "positive csa_window_size."
            )
        if csa_topk_local.dim() != 2 or csa_topk_local.shape[0] < T:
            raise RuntimeError(
                "csa_topk_local must be [T, index_topk] or larger, got "
                f"{tuple(csa_topk_local.shape)} for T={T}."
            )
        if csa_block_tables.dim() != 2:
            raise RuntimeError(
                f"csa_block_tables must be 2-D, got {tuple(csa_block_tables.shape)}"
            )
        if csa_positions.numel() < T or csa_batch_id_per_token.numel() < T:
            raise RuntimeError("CSA fused decode metadata is shorter than T.")
        csa_topk_local = csa_topk_local[:T].contiguous()
        csa_block_tables = csa_block_tables.contiguous()
        csa_positions = csa_positions[:T].contiguous()
        csa_batch_id_per_token = csa_batch_id_per_token[:T].contiguous()
        csa_index_topk = int(csa_topk_local.shape[1])
    else:
        csa_topk_local = kv_indices
        csa_block_tables = kv_indices.reshape(1, -1)
        if ordered_split:
            csa_positions = csa_positions[:T].contiguous()
        else:
            csa_positions = kv_indptr
        csa_batch_id_per_token = kv_indptr
        csa_index_topk = 1
    ...
    _bk, num_warps, num_stages = _kernel_config(block_h)
    if block_k is None:
        block_k = 32 if (quant_tail or packed_tail) else _bk
    qk_scale = float(softmax_scale) * LOG2E
    if kv_splits != 1:
        if fuse_csa_topk:
            raise RuntimeError(
                "split KV decode split-K currently requires pre-packed "
                "kv_indices; fused CSA top-k translation is only implemented "
                f"for kv_splits=1, got kv_splits={kv_splits}."
            )
        if not ordered_split:
            csa_positions = kv_indptr
...
def _sparse_attn_v4_paged_decode_aiter_direct(
    q: torch.Tensor,
    unified_kv: torch.Tensor,
    kv_indices: torch.Tensor,
    kv_indptr: torch.Tensor,
    ...
    """Call aiter sparse decode kernels without aiter's Python wrapper copies.

    The public aiter wrapper allocates its own output and calls ``contiguous()``
    on the index tensors. vLLM already provides graph-stable int32 index
    buffers and an output buffer, so use the underlying kernels directly.
    """
    if _aiter_pa_decode_sparse_kernel is None:
        raise RuntimeError("aiter pa_decode_sparse kernel is unavailable")
    if q.dtype not in (torch.bfloat16, torch.float16):
        raise RuntimeError(f"aiter pa_decode_sparse expects fp16/bf16 q, got {q.dtype}")
    if unified_kv.dtype != q.dtype:
        raise RuntimeError(
            f"aiter pa_decode_sparse dtype mismatch: kv={unified_kv.dtype}, q={q.dtype}"
        )
    if kv_indices.dtype != torch.int32 or kv_indptr.dtype != torch.int32:
        raise RuntimeError("aiter pa_decode_sparse expects int32 indices/indptr")
    if not kv_indices.is_contiguous() or not kv_indptr.is_contiguous():
        raise RuntimeError("aiter pa_decode_sparse expects contiguous metadata")

    T, H, D = q.shape
    if out is None:
        out = torch.empty_like(q)
    elif out.shape != q.shape or out.dtype != q.dtype or out.device != q.device:
        raise RuntimeError("aiter pa_decode_sparse out must match q shape/dtype/device")
    ...
    kv_splits, _ = sparse_attn_v4_paged_decode_kv_splits(
        T,
        H,
        block_h=block_h,
    )
    ...
    _aiter_pa_decode_sparse_kernel[(T, n_head_blocks, kv_splits)](
        q,
        unified_kv,
        kv_indices,
        kv_indptr,
    ...
    _aiter_pa_decode_sparse_reduce_zero_safe[(T, H)](
        m_partial,
        l_partial,
        acc_partial,
        attn_sink,
