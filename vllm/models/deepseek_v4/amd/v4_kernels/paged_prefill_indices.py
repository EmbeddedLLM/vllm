...
per-fwd index buffers consumed by `sparse_attn_v4_paged_prefill`:
  ...
  - ``kv_indices_prefix_swa``   : Dense path — SWA prior-chunk paged offsets
                                  into `unified_kv`.
  - ``kv_indices_prefix_csa``   : CSA path — SWA prior-chunk paged offsets
                                  are written at the slice head; CSA topk is
                                  filled after that prefix by
                                  ``csa_translate_pack``.
...
Caller responsibilities (no copies done here):
  - For CSA, this kernel writes the SWA prefix segment and
    ``csa_translate_pack`` writes the remaining consumed topk segment. No
    sentinel fill is needed for consumed slots.
...
Per-token SWA prefix offsets use the vLLM-owned SWA block table:
  paged[t,k] = block_table[bid, pos // block_size] * block_size
               + pos % block_size
"""
...
def _v4_paged_prefill_indices_kernel(
    ...
    cu_seqlens_q_per_seq_ptr,  # [bs] int — per-seq start in per-fwd kv tensor
    state_slot_per_seq_ptr,  # [bs] int — retained for the public ABI
    n_committed_hca_per_seq_ptr,  # [bs] int — per-seq HCA compress entry count
    swa_block_tables_ptr,  # [bs, MAX_SWA_BLOCKS] int — SWA physical blocks
    swa_bt_stride_bs,
    ...
    # Constants.
    win: tl.constexpr,
    cs,  # retained for the public ABI
    swa_pages,  # flattened SWA row count; boundary into compressed storage
    WRITE_HCA: tl.constexpr,
    ...
    # A prefill token may only see HCA entries whose source block has
    # completed at or before that token. The per-request committed count also
    # includes entries created later in the same scheduled chunk.
    n_hca = tl.minimum(n_hca, (pos + 1) // 128)
    ...
    # paged = swa_block_tables[bid, pos // block_size] * block_size
    #         + pos % block_size
    ...
    logical_blocks = global_pos // SWA_BLOCK_SIZE
    physical_blocks = tl.load(
        swa_block_tables_ptr + bid * swa_bt_stride_bs + logical_blocks,
        mask=swa_mask,
        other=0,
    )
    paged = physical_blocks * SWA_BLOCK_SIZE + global_pos % SWA_BLOCK_SIZE
    ...
    if WRITE_HCA:
        ...
        # block_tables row stride is `bt_stride_bs` int32 elements.
...
def write_v4_paged_prefill_indices(
    *,
    ...
    Caller is responsible for:
      1. Computing the four indptr cumsums (e.g. via ``torch.cumsum`` over
         the per-token count vectors).
      2. Computing ``bid_per_token`` (e.g.
    ...
    Args (all GPU tensors):
      ...
      cu_seqlens_q_per_seq:      ``[bs]``   int — per-seq cu_seqlens_q[bid]
                                            ...
                                            ``bs`` entries).
      state_slot_per_seq:        ``[bs]``   int — retained for ABI compatibility.
      ...
      prefix_csa_indices:        ``[csa_total]`` int OUT — SWA prefix
                                  segment written here; CSA topk segment is
                                  written later by ``csa_translate_pack``.
                                  The full consumed slice is therefore
                                  written without a sentinel prefill.
      ...
      write_hca:                 bool — write HCA compressed entries. Set
                                  false for CSA/SWA layers whose HCA prefix
                                  buffer is not sized for ``n_hca``.
    """
    if T == 0:
        return
...
def write_v4_paged_prefill_indices_reference(
    *,
    ...
    Same caller contract: the CSA slice is fully written by the combination of
    this helper and ``csa_translate_pack``; no sentinel prefill is required for
    consumed slots.
    """
    if T == 0:
        return
    ...
    for t in range(T):
        ...
        cu_q = cu_q_cpu[bid]
        n_hca = min(n_hca_cpu[bid], (pos + 1) // 128)
        ...
        if prefix_swa_count > 0:
            ...
            logical_blocks = torch.div(
                global_pos, swa_block_size, rounding_mode="floor"
            )
            physical_blocks = swa_block_tables_cpu[bid, logical_blocks.cpu()].to(device)
            paged = physical_blocks * swa_block_size + global_pos % swa_block_size
