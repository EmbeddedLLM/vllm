...
if current_platform.is_rocm():
    ...
    # aiter ck op
    def rocm_aiter_gemm_a8w8_ck_impl(
        input: torch.Tensor,
        weight: torch.Tensor,
        out_dtype: torch.dtype | None = None,
        scale_a: torch.Tensor | None = None,
        ...
        n = weight.shape[0]
        Y = torch.empty(m, n, dtype=out_dtype, device=input.device)
        gemm_a8w8_CK(input, weight, scale_a, scale_b, bias, out_dtype, Y)
        return Y

    def rocm_aiter_gemm_a8w8_ck_fake(
        input: torch.Tensor,
        weight: torch.Tensor,
        out_dtype: torch.dtype | None = None,
        scale_a: torch.Tensor | None = None,
    ...
    direct_register_custom_op(
        ...
        op_func=rocm_aiter_gemm_a8w8_ck_impl,
        mutates_args=[],
        fake_impl=rocm_aiter_gemm_a8w8_ck_fake,
        dispatch_key=current_platform.dispatch_key,
    )


def rocm_aiter_per_token_w8a8_scaled_mm_bpreshuffle(
    qinput: torch.Tensor,
    weight: torch.Tensor,
...
# Wrapper for gemm_a8w8_CK (N, K) non-shuffled weight
def rocm_aiter_per_token_w8a8_scaled_mm_ck(
    qinput: torch.Tensor,
    weight: torch.Tensor,
    out_dtype: torch.dtype,
    scale_a: torch.Tensor,
    ...
    output_shape = [*qinput.shape[:-1], weight.shape[0]]
    output = torch.ops.vllm.rocm_aiter_gemm_a8w8_ck(
        qinput, weight, scale_a=scale_a, scale_b=scale_b, out_dtype=out_dtype, bias=bias
    )
