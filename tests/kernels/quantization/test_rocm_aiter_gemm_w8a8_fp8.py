...
def test_rocm_aiter_gemm_a8w8_bpreshuffle_torch_compile_compatibility(
    m: int, nk: tuple[int, int], out_dtype: torch.dtype, quant_dtype: torch.dtype
):
    n, k = nk
    ...
    torch.library.opcheck(
        torch.ops.vllm.rocm_aiter_gemm_a8w8_bpreshuffle,
        (qinput, weight),
        kwargs={
            "scale_a": scale_a,
            "scale_b": scale_b,
    ...
    output = per_token_w8a8_scaled_mm(qinput, weight, scale_a, scale_b, out_dtype)

    # Compile the function with appropriate settings
    compiled_fn = torch.compile(
        per_token_w8a8_scaled_mm,
        fullgraph=True,
        backend="inductor",
...
def test_rocm_aiter_gemm_a8w8_ck_custom_op_registration():
    """Test that the custom op is correctly registered."""
    # Check if the op exists in torch.ops.vllm
    assert hasattr(torch.ops.vllm, "rocm_aiter_gemm_a8w8_ck")
    # Check if the op is callable
    assert callable(torch.ops.vllm.rocm_aiter_gemm_a8w8_ck)
...
@pytest.mark.parametrize("out_dtype", [torch.bfloat16, torch.float16])
@pytest.mark.parametrize("quant_dtype", [torch.float8_e4m3fnuz])
def test_rocm_aiter_gemm_a8w8_ck_torch_compile_compatibility(
    m: int, nk: tuple[int, int], out_dtype: torch.dtype, quant_dtype: torch.dtype
):
    n, k = nk
    qinput = torch.randn((m, k), dtype=out_dtype, device="cuda")
    ...
    # NOTE: No weight shuffle for CK kernel
    # from aiter.ops.shuffle import shuffle_weight
    # weight = shuffle_weight(weight)
    ...
    def per_token_w8a8_scaled_mm(
        ...
        return torch.ops.vllm.rocm_aiter_gemm_a8w8_ck(
            qinput,
            weight,
            scale_a=scale_a,
            scale_b=scale_b,
    ...
    # Verify the op's fake implementation
    torch.library.opcheck(
        torch.ops.vllm.rocm_aiter_gemm_a8w8_ck,
        (qinput, weight),
        kwargs={
            "scale_a": scale_a,
            "scale_b": scale_b,
    ...
    assert torch.allclose(output, output_compiled, atol=1e-2, rtol=1e-2)
