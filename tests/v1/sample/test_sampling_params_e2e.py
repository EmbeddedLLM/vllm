...
def test_bad_words(llm):
    ...
    def validate_bad_word(llm, bad_word, output_tokens):
        # Validate that the bad word tokens are in the output so that
        # the filter can properly reject the bad word in the next query.
        # For the same text, tokenization generated by the tokenizer
        # may differ from the model output, so to effectively test the
        # bad word filter, we need to make sure that the bad word we
        # use can be tokenized in the same way as the model output.
        # If the given bad word fails this condition, we create a new
        # bad word from the model output.
        def list_contains_slice(list_in, slice_in):
            for i in range(len(list_in) - len(slice_in) + 1):
                if list_in[i : i + len(slice_in)] == slice_in:
                    return True
            return False

        bad_word_tokens = tokenizer.encode(text=bad_word, add_special_tokens=False)
        return list_contains_slice(output_tokens, bad_word_tokens)

    for i in range(0, len(split_text) - 1):
        bad_words_1 = " ".join(split_text[i : i + 2])
        if validate_bad_word(llm, bad_words_1, output[0].outputs[0].token_ids):
            break
    else:
        raise ValueError(
            "No valid multi-word bad word found. "
            "Consider generating a longer response or using a different prompt."
        )
    ...
    for i in range(len(split_text) - 1, -1, -1):
        bad_words_2 = split_text[i]
        if validate_bad_word(llm, bad_words_2, output[0].outputs[0].token_ids):
            break
    else:
        raise ValueError(
            "No valid single-word bad word found. "
            "Consider generating a longer response or using a different prompt."
        )
